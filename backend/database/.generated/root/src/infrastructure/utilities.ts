		...
		if (typeof input === "object") {
			...
			for (let i = 0; i < keyLength; i++) {
				...
				if (value instanceof mongoose.Types.ObjectId) {
					input[keys[i]] = input[keys[i]].toString();
				}
				else if (typeof value === "object") {
					input[keys[i]] = objectIdToString(value);
				}
			}
		}
	}
...
export function generateSet(input, single, list, parent: boolean = true, parentKey: string = null) {
		...
		for (let i = 0; i < keyLength; i++) {
			...
			if (Array.isArray(value)) {
				...
				if (setSpeacialKey.includes(lastChar)) {
					...
					if (lastChar === "-") {
						list["$pull"][key] = { $in: value };
					}
					else if (lastChar === "+") {
						list["$addToSet"][key] = value;
					}
					else {
						single[key] = value;
					}
	...
	if (parent) {
		...
		if (single && Object.keys(single).length > 0) {
			setObject["$set"] = single;
		}
		if (list && Object.keys(list).length > 0) {
			if(Object.keys(list["$pull"]).length > 0){
				setObject["$pull"] = list["$pull"];
			}
			if(Object.keys(list["$addToSet"]).length > 0){
				setObject["$addToSet"] = list["$addToSet"];
			}
		}
