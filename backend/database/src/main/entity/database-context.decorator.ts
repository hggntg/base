...
import { getClass, getMetadata, Property, getProperties } from "@base/class";
import { App } from "@base/interfaces";

declare const app: App & extendDatabase;

export interface IDbContextProperty {
...
function getDbContext(target: any) : IDbContextProperty{
	let classImp = getClass(target);
	let dbContext = getMetadata(DBCONTEXT_KEY, classImp);
	return dbContext;
}

export class DbContextPropertyImp implements IDbContextProperty {
	connection: mongoose.Connection;
...
export function DBContext(uri: string, connectionOptions: mongoose.ConnectionOptions) {
	return function (target: object) {
		...
		dbContext.connection = mongoose.createConnection(uri, connectionOptions);
		app.use(new Promise((resolve, reject) => {
			mongoose.createConnection(uri, connectionOptions).then(connection => {
		...
		Reflect.defineMetadata(DBCONTEXT_KEY, dbContext, target.constructor);
	}
}
...
export class CollectionImp<T extends BaseEntity> implements ICollection<T>{
	find(conditions: any = {}) {
...
export function Collection<T extends BaseEntity>(classImp: {new() : T}) {
